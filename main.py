...
def is_metal_tender(title: str) -> bool:
    """
    Tenderplan уже ищет по нашим ключевым словам.
    Поэтому на этапе диагностики НЕ режем результаты по названию:
    API может вернуть релевантный тендер, где в названии нет слова "лом".
    Отсекаем только явно чужие закупки.
    """
    if not title:
        return False
    t = title.lower()
